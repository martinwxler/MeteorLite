...
public final class ObjectSound extends Node {
  ...
  static int method1889(int var0, Script var1, boolean var2) {
    ...
    if (var0 >= 2000) { // L: 846
      var0 -= 1000; // L: 847
      var4 = WorldMapData_1.getWidget(
          Interpreter.Interpreter_intStack[--ChatChannel.Interpreter_intStackSize]); // L: 848
      var3 = false; // L: 849
    } else {
    ...
    if (var0 == ScriptOpcodes.CC_SETOP) { // L: 852
      var11 =
          Interpreter.Interpreter_intStack[--ChatChannel.Interpreter_intStackSize] - 1; // L: 853
      if (var11 >= 0 && var11 <= 9) { // L: 854
        var4.setAction(var11,
            Interpreter.Interpreter_stringStack[--UserComparator9.Interpreter_stringStackSize]); // L: 858
        return 1; // L: 859
      } else {
      ...
      if (var0 == ScriptOpcodes.CC_SETDRAGGABLE) { // L: 861
        ...
        var4.isScrollBar =
            Interpreter.Interpreter_intStack[--ChatChannel.Interpreter_intStackSize] == 1; // L: 869
        return 1; // L: 870
        ...
        var4.prioritizeMenuEntry =
            Interpreter.Interpreter_intStack[--ChatChannel.Interpreter_intStackSize] == 1; // L: 893
        return 1; // L: 894
        ...
        if (var0 != ScriptOpcodes.CC_SETOPKEY) { // L: 900
          ...
          if (var0 == ScriptOpcodes.CC_SETOPTKEY) { // L: 930
            ...
            var5 = 10; // L: 932
            var8 = new byte[]{
                (byte) Interpreter.Interpreter_intStack[ChatChannel.Interpreter_intStackSize]}; // L: 933
            byte[] var9 = new byte[]{
                (byte) Interpreter.Interpreter_intStack[ChatChannel.Interpreter_intStackSize
                    + 1]}; // L: 934
          ...
          if (var3) { // L: 903
            ...
            for (var7 = 0; var7 < 10
                && Interpreter.Interpreter_intStack[var7 + ChatChannel.Interpreter_intStackSize]
                >= 0; var7 += 2) { // L: 906 907
            }
            ...
            if (var7 > 0) { // L: 909
              ...
              for (var7 -= 2; var7 >= 0; var7 -= 2) { // L: 912
                var10[var7 / 2] = (byte) Interpreter.Interpreter_intStack[var7
                    + ChatChannel.Interpreter_intStackSize]; // L: 913
                var8[var7 / 2] = (byte) Interpreter.Interpreter_intStack[var7
                    + ChatChannel.Interpreter_intStackSize + 1]; // L: 914
              }
            }
          } else {
            ChatChannel.Interpreter_intStackSize -= 2; // L: 919
            var10 = new byte[]{
                (byte) Interpreter.Interpreter_intStack[ChatChannel.Interpreter_intStackSize]}; // L: 920
            var8 = new byte[]{
                (byte) Interpreter.Interpreter_intStack[ChatChannel.Interpreter_intStackSize
                    + 1]}; // L: 921
          }
