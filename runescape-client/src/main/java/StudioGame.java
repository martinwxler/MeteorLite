...
public enum StudioGame implements Enumerated {
  ...
  static int method4983(int var0, Script var1, boolean var2) {
    if (var0 == ScriptOpcodes.SOUND_SYNTH) { // L: 1824
      ...
      ClanSettings
          .queueSoundEffect(Interpreter.Interpreter_intStack[ChatChannel.Interpreter_intStackSize],
      ...
      class6.playSong(
          Interpreter.Interpreter_intStack[--ChatChannel.Interpreter_intStackSize]); // L: 1830
      return 1; // L: 1831
    } else if (var0 == ScriptOpcodes.SOUND_JINGLE) { // L: 1833
      ChatChannel.Interpreter_intStackSize -= 2; // L: 1834
      ReflectionCheck
          .method1230(Interpreter.Interpreter_intStack[ChatChannel.Interpreter_intStackSize],
              Interpreter.Interpreter_intStack[ChatChannel.Interpreter_intStackSize
                  + 1]); // L: 1835
      return 1; // L: 1836
    } else {
      ...
      if (var0 == 3203) { // L: 1838
        var3 = Math.min(
            Math.max(Interpreter.Interpreter_intStack[--ChatChannel.Interpreter_intStackSize], 0),
            100); // L: 1839
        class0.method0(Math.round(2.55F * (float) var3)); // L: 1840
        return 1; // L: 1841
      } else if (var0 == 3204) { // L: 1843
        Interpreter.Interpreter_intStack[++ChatChannel.Interpreter_intStackSize - 1] = Math
            .round((float) class4.clientPreferences.musicVolume / 2.55F); // L: 1844
        return 1; // L: 1845
        ...
        Interpreter.Interpreter_intStack[++ChatChannel.Interpreter_intStackSize - 1] = Math
            .round((float) class4.clientPreferences.soundEffectsVolume / 1.27F); // L: 1853
        return 1; // L: 1854
        ...
        Interpreter.Interpreter_intStack[++ChatChannel.Interpreter_intStackSize - 1] = Math
            .round((float) class4.clientPreferences.areaSoundEffectsVolume / 1.27F); // L: 1862
        return 1; // L: 1863
      } else {
        return 2; // L: 1865
      }
