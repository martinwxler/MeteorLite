...
public abstract class AbstractWorldMapIcon {
  ...
  static final void checkIfMinimapClicked(Widget var0, int var1, int var2) {
    if (Client.minimapState == 0 || Client.minimapState == 3) { // L: 3812
      if (!Client.isMenuOpen && (MouseHandler.MouseHandler_lastButton == 1
          || !UserComparator3.mouseCam && MouseHandler.MouseHandler_lastButton == 4)) { // L: 3813
        ...
        if (var3.contains(var4, var5)) { // L: 3818
          ...
          PacketBufferNode var13 = InterfaceParent.getPacketBufferNode(ClientPacket.field2615,
              Client.packetWriter.isaacCipher); // L: 3829
          var13.packetBuffer.writeByte(18); // L: 3830
          var13.packetBuffer.writeShort(var12 + WorldMapSprite.baseY); // L: 3831
          var13.packetBuffer.method6639(
              KeyHandler.KeyHandler_pressedKeys[82] ? (KeyHandler.KeyHandler_pressedKeys[81] ? 2
                  : 1) : 0); // L: 3832
  ...
  boolean labelFitsScreen(int var1, int var2) {
    ...
    if (var3 == null) { // L: 80
      return false; // L: 81
    } else if (var1 >= this.screenX - var3.width / 2
        && var1 <= var3.width / 2 + this.screenX) { // L: 83
