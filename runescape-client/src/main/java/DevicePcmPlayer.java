...
public class DevicePcmPlayer extends PcmPlayer {
  ...
  @Export("init")
  protected void init() {
    this.format = new AudioFormat((float) class341.field3921, 16,
        PcmPlayer.PcmPlayer_stereo ? 2 : 1, true, false); // L: 21
  ...
  protected void open(int var1) throws LineUnavailableException {
    try {
      Info var2 = new Info(SourceDataLine.class, this.format,
          var1 << (PcmPlayer.PcmPlayer_stereo ? 2 : 1)); // L: 27
  ...
  protected int position() {
    return this.capacity2 - (this.line.available() >> (PcmPlayer.PcmPlayer_stereo ? 2
        : 1)); // L: 44
  }
