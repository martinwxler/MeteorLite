    ...
    try {
      if (class233.musicPlayerStatus == 2) { // L: 104
        if (ArchiveLoader.musicTrack == null) { // L: 105
          ArchiveLoader.musicTrack = MusicTrack
              .readTrack(InvDefinition.musicTrackArchive, class233.musicTrackGroupId,
                  class233.musicTrackFileId); // L: 106
        ...
        if (class215.soundCache == null) { // L: 109
          class215.soundCache = new SoundCache(class230.soundEffectsArchive,
              class233.musicSamplesArchive);
        }

        if (class233.midiPcmStream
            .loadMusicTrack(ArchiveLoader.musicTrack, class233.musicPatchesArchive,
                class215.soundCache, 22050)) { // L: 110
          class233.midiPcmStream.clearAll(); // L: 111
          class233.midiPcmStream.setPcmStreamVolume(class233.musicTrackVolume); // L: 112
          class233.midiPcmStream.setMusicTrack(ArchiveLoader.musicTrack,
              GrandExchangeOfferUnitPriceComparator.musicTrackBoolean); // L: 113
