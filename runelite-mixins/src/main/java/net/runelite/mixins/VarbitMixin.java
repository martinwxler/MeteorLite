...
public abstract class VarbitMixin implements RSClient {
  ...
  public RSVarbitComposition getVarbitComposition(int id) {
    ...
    if (varbit == null) {
      ...
      varbit = (RSVarbitComposition) getVarbitCache().get$api(id);
      varbitCache.put(id, varbit);
    }

    return varbit.getIndex() == 0 && varbit.getLeastSignificantBit() == 0
        && varbit.getMostSignificantBit() == 0 ? null : varbit;
  }
  ...
  public int getVarbitValue(int[] varps, int varbitId) {
    ...
    if (v == null) {
      throw new IndexOutOfBoundsException(
          "Varbit " + varbitId + " does not exist!"); // oob for "backwards compatibility lol"
    }
  ...
  public void setVarbitValue(int[] varps, int varbitId, int value) {
    ...
    if (v == null) {
      throw new IndexOutOfBoundsException(String
          .format("Varbit %d does not exist!", varbitId)); // oob for "backwards compatibility lol"
    }
