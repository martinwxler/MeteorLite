      ...
      try {
        ...
        if (is != null) {
          ...
          for (String s : files) {
            ...
            InputStream hashStream = ClassLoader.getSystemClassLoader()
                .getResourceAsStream("scripts/" + scriptName + ".hash");
            if (hashStream != null) {
              String scriptHash = (String) IOUtils.readLines(hashStream, Charsets.UTF_8)
                  .toArray()[0];
    ...
    if (rsData != null) {
      ...
      if (!overlayHash.equalsIgnoreCase(originalHash)) {
        log.error("Client script " + groupId
            + " hash is invalid. The script will not be overridden. This will break plugin(s)!" + " : " + originalHash);
