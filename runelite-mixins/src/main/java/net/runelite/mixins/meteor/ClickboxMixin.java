...
public abstract class ClickboxMixin implements RSClient {
  ...
  @Inject
  public void checkClickbox(Model rlModel, int orientation, int pitchSin, int pitchCos, int yawSin,
    ...
    if (!hasFlag || !viewportContainsMouse || (client.getOculusOrbState() != 0 && !client
  ...
  private boolean boundingboxCheck(Model model, int x, int y, int z) {
    ...
    if (Math.abs(var44) > var41 + field1722) {
      ...
      passes = false;
    } else if (Math.abs(var46 * field638 - var45 * field1846)
        > var42 * field38 + var43 * field601) {
      passes = false;
    } else if (Math.abs(var44 * field1846 - var46 * field1720)
        > var43 * field1722 + var41 * field38) {
      passes = false;
    } else if (Math.abs(var45 * field1720 - var44 * field638)
        > var42 * field1722 + var41 * field601) {
      passes = false;
    } else {
      passes = true;
    }
