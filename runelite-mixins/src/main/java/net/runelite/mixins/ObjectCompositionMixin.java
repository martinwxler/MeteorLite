...
public abstract class ObjectCompositionMixin implements RSObjectComposition
{
	@Shadow("client")
	...
	public void decodeNext(RSBuffer buffer, int opcode)
	{
		...
		else if (opcode == 2) setName(buffer.readStringCp1252NullTerminated$api());
		else if (opcode == 5)
		{
			var3 = buffer.readUnsignedByte$api();
			if (var3 > 0)
		...
		{
			getActions()[opcode - 30] = buffer.readStringCp1252NullTerminated$api();
