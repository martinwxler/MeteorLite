...
public class UpdateMappings {
  ...
  public static void main(String args[]) {
    ...
    scanGamepack(deob);
    System.out.println(
        "Found " + found + "/" + (StaticMethods.expected.size() + StaticFields.expected.size())
    ...
    try (ZipFile zf = new ZipFile(f)) {
      ...
      while (entries.hasMoreElements()) {
        ...
        if (entry.getName().endsWith(".class")) {
          if (!entry.getName().contains("/")) // Only check deob classes
          {
            continue;
          }
  ...
  public static void buildMappingField(String fieldName, String refClassName) {
    mappings
        .append("   public static final String " + fieldName + " = \"" + refClassName + "\";\n");
    found++;
  }
}
