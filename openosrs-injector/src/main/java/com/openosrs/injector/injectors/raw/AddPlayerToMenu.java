...
public class AddPlayerToMenu extends AbstractInjector {
  ...
  public void inject() {
    ...
    final net.runelite.asm.pool.Method shouldHideAttackOptionFor =
        inject.getVanilla().findClass("client").findMethod("shouldHideAttackOptionFor")
            .getPoolMethod();
  ...
  private void injectSameTileFix(Method addPlayerOptions,
      net.runelite.asm.pool.Method shouldDrawMethod) {
    // ALOAD 0
    // ICONST_0
    ...
    List<Instruction> prependList = new ArrayList<Instruction>() {{
  ...
  private void injectHideAttack(Method addPlayerOptions,
      net.runelite.asm.pool.Method shouldHideAttackOptionFor) {
    final Field AttackOption_hidden =
        InjectUtil.findField(inject, "AttackOption_hidden", "AttackOption").getPoolField();
    final Field attackOption = InjectUtil.findField(inject, "playerAttackOption", "Client")
        .getPoolField();
  ...
  private void injectHideCast(Method addPlayerOptions,
      net.runelite.asm.pool.Method shouldHideAttackOptionFor) {
    // LABEL before
    // BIPUSH 8
    // LDC (garbage)
      ...
      if (!(i instanceof JumpingInstruction)) {
        if (b1 && b2 && iAnd && getstatic) {
          throw new InjectException(
              "@ me in discord if this shit is broken lol, hide spells failed btw");
        }
        continue;
      }
