...
public class DrawMenu extends AbstractInjector {
  ...
  public void inject() {
    ...
    final Method drawLoggedIn = InjectUtil
        .findMethod(inject, "drawLoggedIn", "Client", null, true, false);
    final Field gameDrawMode = InjectUtil.findField(inject, "gameDrawingMode", "Client")
        .getPoolField();
    ...
    for (InstructionContext ic : mc.getInstructionContexts()) {
      ...
      if (((GetStatic) instruction).getField().equals(isMenuOpen)) {
        ...
        assert isMenuOpenPopI instanceof IfEq || isMenuOpenPopI instanceof IfNe :
            "isMenuOpen was popped by a " + isMenuOpenPopI + "?";
