...
public class MethodNode extends MethodVisitor {
    ...
    public void check(int api) {
        if (api == 262144) {
            ...
            for (i = 0; i < this.instructions.size(); ++i) {
                AbstractInsnNode var6 = this.instructions.get(i);
                ...
                if (var6 instanceof MethodInsnNode) {
                    boolean itf = ((MethodInsnNode) var6).itf;
                    if (itf != (var6.opcode == 185)) {
                        throw new RuntimeException();
                    }
                }
            }
    ...
    public void accept(MethodVisitor mv) {
        ...
        int i;
        for (i = 0; i < n; ++i) {
            ParameterNode l = (ParameterNode) this.parameters.get(i);
            mv.visitParameter(l.name, l.access);
        }

        if (this.annotationDefault != null) {
            AnnotationVisitor var7 = mv.visitAnnotationDefault();
        ...
        AnnotationNode var8;
        for (i = 0; i < n; ++i) {
            var8 = (AnnotationNode) this.visibleAnnotations.get(i);
            var8.accept(mv.visitAnnotation(var8.desc, true));
        }

        n = this.invisibleAnnotations == null ? 0 : this.invisibleAnnotations.size();

        for (i = 0; i < n; ++i) {
            var8 = (AnnotationNode) this.invisibleAnnotations.get(i);
            var8.accept(mv.visitAnnotation(var8.desc, false));
        }
        ...
        for (i = 0; i < n; ++i) {
            var9 = (TypeAnnotationNode) this.visibleTypeAnnotations.get(i);
            var9.accept(mv.visitTypeAnnotation(var9.typeRef, var9.typePath, var9.desc, true));
        }

        n = this.invisibleTypeAnnotations == null ? 0 : this.invisibleTypeAnnotations.size();

        for (i = 0; i < n; ++i) {
            var9 = (TypeAnnotationNode) this.invisibleTypeAnnotations.get(i);
            var9.accept(mv.visitTypeAnnotation(var9.typeRef, var9.typePath, var9.desc, false));
        }
        ...
        List var10;
        for (i = 0; i < n; ++i) {
            var10 = this.visibleParameterAnnotations[i];
            if (var10 != null) {
                for (j = 0; j < var10.size(); ++j) {
                    an = (AnnotationNode) var10.get(j);
        ...
        for (i = 0; i < n; ++i) {
            var10 = this.invisibleParameterAnnotations[i];
            if (var10 != null) {
                for (j = 0; j < var10.size(); ++j) {
                    an = (AnnotationNode) var10.get(j);
