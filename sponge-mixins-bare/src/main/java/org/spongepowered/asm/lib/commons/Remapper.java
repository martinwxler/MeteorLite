...
public abstract class Remapper {
    ...
    public String mapDesc(String desc) {
        ...
        switch (t.getSort()) {
            case 9:
                ...
                for (int var5 = 0; var5 < t.getDimensions(); ++var5) {
                    s = '[' + s;
                }
    ...
    private Type mapType(Type t) {
        ...
        switch (t.getSort()) {
            case 9:
                s = this.mapDesc(t.getElementType().getDescriptor());
    ...
    public String mapMethodDesc(String desc) {
        if ("()V".equals(desc)) {
            ...
            for (int returnType = 0; returnType < args.length; ++returnType) {
                sb.append(this.mapDesc(args[returnType].getDescriptor()));
            }

            Type var5 = Type.getReturnType(desc);
            if (var5 == Type.VOID_TYPE) {
                sb.append(")V");
                return sb.toString();
            } else {
                sb.append(')').append(this.mapDesc(var5.getDescriptor()));
                return sb.toString();
            }
        }
    }
