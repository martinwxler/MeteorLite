...
public class SourceInterpreter extends Interpreter implements Opcodes {
    ...
    @Override
    public SourceValue copyOperation(AbstractInsnNode insn, Value value) {
        return new SourceValue(value.getSize(), insn);
    }

    @Override
    public SourceValue unaryOperation(AbstractInsnNode insn, Value value) {
        int size;
        switch (insn.getOpcode()) {
            case 117:
            case 119:
    ...
    @Override
    public SourceValue binaryOperation(AbstractInsnNode insn, Value value1, Value value2) {
        byte size;
        switch (insn.getOpcode()) {
            case 47:
            case 49:
    ...
    @Override
    public SourceValue ternaryOperation(AbstractInsnNode insn, Value value1, Value value2, Value value3) {
    ...
    @Override
    public SourceValue merge(Value dO, Value wO) {
        SourceValue d = (SourceValue) dO;
        SourceValue w = (SourceValue) wO;
        if (d.insns instanceof SmallSet && w.insns instanceof SmallSet) {
            Set s1 = ((SmallSet) d.insns).union((SmallSet) w.insns);
            return s1 == d.insns && d.size == w.size ? d : new SourceValue(Math.min(d.size, w.size), s1);
