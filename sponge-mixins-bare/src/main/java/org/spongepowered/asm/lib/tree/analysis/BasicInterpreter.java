...
public class BasicInterpreter extends Interpreter implements Opcodes {
    ...
    @Override
    public BasicValue unaryOperation(AbstractInsnNode insn, Value value) throws AnalyzerException {
        String desc;
        switch (insn.getOpcode()) {
            case 116:
            case 132:
    ...
    @Override
    public BasicValue binaryOperation(AbstractInsnNode insn, Value value1, Value value2) throws AnalyzerException {
        switch (insn.getOpcode()) {
            case 46:
            case 51:
            case 52:
    ...
    @Override
    public BasicValue ternaryOperation(AbstractInsnNode insn, Value value1, Value value2, Value value3) throws AnalyzerException {
        return null;
    }
    ...
    public BasicValue naryOperation(AbstractInsnNode insn, List values) throws AnalyzerException {
        ...
        return opcode == 197 ? this.newValue(Type.getType(((MultiANewArrayInsnNode) insn).desc)) : (opcode == 186 ? this.newValue(Type.getReturnType(((InvokeDynamicInsnNode) insn).desc)) : this.newValue(Type.getReturnType(((MethodInsnNode) insn).desc)));
    }
    ...
    public BasicValue merge(Value v, Value w) {
        return !v.equals(w) ? BasicValue.UNINITIALIZED_VALUE : (BasicValue) v;
    }
}
