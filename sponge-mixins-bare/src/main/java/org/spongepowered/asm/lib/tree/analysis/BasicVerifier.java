    ...
    public BasicValue naryOperation(AbstractInsnNode insn, List values) throws AnalyzerException {
        ...
        if (opcode == 197) {
            ...
            if (opcode != 184 && opcode != 186) {
                Type desc = Type.getObjectType(((MethodInsnNode) insn).owner);
                if (!this.isSubTypeOf((BasicValue) values.get(i++), this.newValue(desc))) {
                    throw new AnalyzerException(insn, "Method owner", this.newValue(desc), (Value) values.get(0));
                }
            }

            String var10 = opcode == 186 ? ((InvokeDynamicInsnNode) insn).desc : ((MethodInsnNode) insn).desc;
            Type[] args = Type.getArgumentTypes(var10);
