...
public class IdeaNotNullHelper {
  ...
  public static boolean removeHardcodedChecks(Statement root, StructMethod mt) {
    ...
    while (findAndRemoveParameterCheck(root,
        mt)) { // iterate until nothing found. Each invocation removes one parameter check.
      checks_removed = true;
    }

    // method @NotNull annotation
    while (findAndRemoveReturnCheck(root,
        mt)) { // iterate until nothing found. Each invocation handles one method exit check.
      checks_removed = true;
    }
    ...
    if (st.type == Statement.TYPE_IF) {
      ...
      if (ifbranch != null &&
        ...
        if (second_param.type == Exprent.EXPRENT_CONST &&
            second_param.getExprType().type
                == CodeConstants.TYPE_NULL) { // TODO: reversed parameter order
          if (first_param.type == Exprent.EXPRENT_VAR) {
            ...
            StructAnnotationParameterAttribute param_annotations =
                (StructAnnotationParameterAttribute) mt.getAttribute(
                    StructGeneralAttribute.ATTRIBUTE_RUNTIME_INVISIBLE_PARAMETER_ANNOTATIONS);
            if (param_annotations != null) {

              List<List<AnnotationExprent>> param_annotations_lists = param_annotations
                  .getParamAnnotations();
  ...
  private static boolean removeReturnCheck(Statement stat, StructMethod mt) {
    ...
    if (parent != null && parent.type == Statement.TYPE_IF && stat.type == Statement.TYPE_BASICBLOCK
        ...
        if (exit_exprent.getExitType() == ExitExprent.EXIT_RETURN) {
          ...
          if (ifparent.getElsestat() == stat && if_condition.type == Exprent.EXPRENT_FUNCTION &&
              ((FunctionExprent) if_condition).getFuncType()
                  == FunctionExprent.FUNCTION_EQ) { // TODO: reversed order possible (in theory)
            ...
            if (second_param.type == Exprent.EXPRENT_CONST &&
              ...
              if (first_param.equals(
                  exprent_value)) {        // TODO: check for absence of side effects like method invocations etc.
              ...
              if (second_param.type == Exprent.EXPRENT_CONST &&
                ...
                if (first_param.equals(
                  ...
                  if (ifbranch.type == Statement.TYPE_BASICBLOCK &&
                    ...
                    ifstat.removeSuccessor(
                        ifstat.getAllSuccessorEdges().get(0)); // remove 'else' edge
