...
public class SimplifyExprentsHelper {
  ...
  private static int isArrayInitializer(List<Exprent> list, int index) {
    ...
    if (current.type == Exprent.EXPRENT_ASSIGNMENT) {
      ...
      if (as.getRight().type == Exprent.EXPRENT_NEW && as.getLeft().type == Exprent.EXPRENT_VAR) {
        ...
        if (newex.getExprType().arrayDim > 0 && newex.getLstDims().size() == 1 && newex
            .getLstArrayElements().isEmpty() &&
  ...
  private static boolean isStackAssignement2(Exprent first,
      Exprent second) {  // e.g. 1.4-style class invocation
  ...
  private static boolean isIPPorIMM(Exprent first, Exprent second) {
    ...
    if (first.type == Exprent.EXPRENT_ASSIGNMENT && second.type == Exprent.EXPRENT_FUNCTION) {
      ...
      if ((in.getFuncType() == FunctionExprent.FUNCTION_MMI
          || in.getFuncType() == FunctionExprent.FUNCTION_PPI) &&
  ...
  private static boolean isMonitorExit(Exprent first) {
    if (first.type == Exprent.EXPRENT_MONITOR) {
      ...
      if (monexpr.getMonType() == MonitorExprent.MONITOR_EXIT
  ...
  private static boolean isQualifiedNewGetClass(Exprent first, Exprent second) {
    ...
    if (first.type == Exprent.EXPRENT_INVOCATION) {
      ...
      if (!invexpr.isStatic() && invexpr.getInstance().type == Exprent.EXPRENT_VAR && invexpr
        ...
        for (Exprent expr : lstExprents) {
          if (expr.type == Exprent.EXPRENT_NEW) {
            ...
            if (nexpr.getConstructor() != null && !nexpr.getConstructor().getLstParameters()
                .isEmpty() &&
      ...
      if (as.getLeft().type == Exprent.EXPRENT_VAR && as.getRight().type == Exprent.EXPRENT_NEW) {
        ...
        if (newtype.type == CodeConstants.TYPE_OBJECT && newtype.arrayDim == 0
          ...
          for (int i = index + 1; i < list.size(); i++) {
            ...
            if (setVars.contains(
                leftPaar)) { // variable used somewhere in between -> exit, need a better reduced code
              return false;
            }
          }
        }
  ...
  private static Exprent isLambda(Exprent exprent, StructClass cl) {
    ...
    if (exprent.type == Exprent.EXPRENT_INVOCATION) {
      ...
      if (in.getInvocationTyp() == InvocationExprent.INVOKE_DYNAMIC) {
        ...
        ClassNode lambda_class = DecompilerContext.getClassProcessor().getMapRootClasses()
            .get(lambda_class_name);

        if (lambda_class
            != null) { // real lambda class found, replace invocation with an anonymous class
  ...
  private static Exprent isSimpleConstructorInvocation(Exprent exprent) {
    ...
    if (exprent.type == Exprent.EXPRENT_INVOCATION) {
      InvocationExprent in = (InvocationExprent) exprent;
      if (in.getFunctype() == InvocationExprent.TYP_INIT
          && in.getInstance().type == Exprent.EXPRENT_NEW) {
    ...
    if (stat.type == Statement.TYPE_IF && stat.getExprents() == null) {
      ...
      if (stif.iftype == IfStatement.IFTYPE_IFELSE) {
        ...
        if (ifstat.getExprents() != null && ifstat.getExprents().size() == 1
            ...
            && ifstat.getAllSuccessorEdges().size() == 1
            && elsestat.getAllSuccessorEdges().size() == 1
            && ifstat.getAllSuccessorEdges().get(0).getDestination() == elsestat
            .getAllSuccessorEdges().get(0).getDestination()) {

          Exprent ifexpr = ifstat.getExprents().get(0);
          Exprent elseexpr = elsestat.getExprents().get(0);

          if (ifexpr.type == Exprent.EXPRENT_ASSIGNMENT
              && elseexpr.type == Exprent.EXPRENT_ASSIGNMENT) {
            AssignmentExprent ifas = (AssignmentExprent) ifexpr;
            AssignmentExprent elseas = (AssignmentExprent) elseexpr;

            if (ifas.getLeft().type == Exprent.EXPRENT_VAR
                && elseas.getLeft().type == Exprent.EXPRENT_VAR) {
              VarExprent ifvar = (VarExprent) ifas.getLeft();
              VarExprent elsevar = (VarExprent) elseas.getLeft();

              if (ifvar.getIndex() == elsevar.getIndex() && ifvar
                  .isStack()) { // ifvar.getIndex() >= VarExprent.STACK_BASE) {

                boolean found = false;

                for (Entry<VarVersionPair, FastSparseSet<Integer>> ent : ssa.getPhi().entrySet()) {
                  if (ent.getKey().var == ifvar.getIndex()) {
                    if (ent.getValue().contains(ifvar.getVersion()) && ent.getValue()
                        .contains(elsevar.getVersion())) {
                      found = true;
                      break;
                    }
                  }
                ...
                if (found) {
                  ...
                  data.add(
                      new AssignmentExprent(ifvar, new FunctionExprent(FunctionExprent.FUNCTION_IIF,
                          Arrays.asList(
            ...
            if (ifex.getExitType() == elseex.getExitType() && ifex.getValue() != null
                && elseex.getValue() != null &&
              ...
              if (ifex.getExitType() == ExitExprent.EXIT_THROW &&
                  !ifex.getValue().getExprType().equals(elseex.getValue()
                      .getExprType())) {  // note: getExprType unreliable at this point!
                return false;
              }
              ...
              data.add(new ExitExprent(ifex.getExitType(),
                  new FunctionExprent(FunctionExprent.FUNCTION_IIF,
                      Arrays.asList(
                          stif.getHeadexprent().getCondition(),
                          ifex.getValue(),
                          elseex.getValue()), ifheadexpr_bytecode), ifex.getRetType(),
                  ifheadexpr_bytecode));
              stif.setExprents(data);
  ...
  private static boolean isIff(Exprent exp) {
    return exp.type == Exprent.EXPRENT_FUNCTION
        && ((FunctionExprent) exp).getFuncType() == FunctionExprent.FUNCTION_IIF;
  }
  ...
  private static boolean collapseInlinedClass14(Statement stat) {
    ...
    if (ret) {
      ...
      AssignmentExprent assfirst = (AssignmentExprent) class14Builder
          .getVariableValue("$assignfield$");
      FieldExprent fieldexpr = (FieldExprent) class14Builder.getVariableValue("$field$");

      assfirst.replaceExprent(assfirst.getRight(),
          new ConstExprent(VarType.VARTYPE_CLASS, class_name, null));
      ...
      ClassWrapper wrapper = (ClassWrapper) DecompilerContext
          .getProperty(DecompilerContext.CURRENT_CLASS_WRAPPER);
      if (wrapper != null) {
        wrapper.getHiddenMembers().add(InterpreterUtil
            .makeUniqueKey(fieldexpr.getName(), fieldexpr.getDescriptor().descriptorString));
      }
  ...
  public boolean simplifyStackVarsStatement(Statement stat, HashSet<Integer> setReorderedIfs,
    ...
    if (stat.getExprents() == null) {
      ...
      boolean processClass14 = DecompilerContext
          .getOption(IFernflowerPreferences.DECOMPILE_CLASS_1_4);
