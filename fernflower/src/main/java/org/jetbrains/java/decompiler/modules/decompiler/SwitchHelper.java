...
public class SwitchHelper {
  ...
  public static void simplify(SwitchStatement switchStatement) {
    ...
    if (isEnumArray(value)) {
      ...
      if (classNode != null) {
        MethodWrapper wrapper = classNode.getWrapper()
            .getMethodWrapper(CodeConstants.CLINIT_NAME, "()V");
          ...
          wrapper.getOrBuildGraph().iterateExprents(new DirectGraph.ExprentIterator() {
            ...
            public int processExprent(Exprent exprent) {
              if (exprent instanceof AssignmentExprent) {
                ...
                if (left.type == Exprent.EXPRENT_ARRAY && ((ArrayExprent) left).getArray()
                    .equals(arrayField)) {
                  mapping.put(assignment.getRight(),
                      ((InvocationExprent) ((ArrayExprent) left).getIndex()).getInstance());
                }
              }
              return 0;
            }
          ...
          if (exprent == null) {
            ...
            if (realConst == null) {
              DecompilerContext.getLogger()
                  .writeMessage(
                      "Unable to simplify switch on enum: " + exprent + " not found, available: "
                          + mapping,
      ...
      switchExprent
          .replaceExprent(value, ((InvocationExprent) array.getIndex()).getInstance().copy());
    }
  }
  ...
  private static boolean isEnumArray(Exprent exprent) {
    if (exprent instanceof ArrayExprent) {
      ...
      return field instanceof FieldExprent && ((FieldExprent) field).getName()
