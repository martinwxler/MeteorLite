...
public class FinallyProcessor {
  ...
  private static void insertSemaphore(ControlFlowGraph graph,
      ...
      for (BasicBlock dest : lstSucc) {
        ...
        if (!setCopy.contains(dest) && dest != graph.getLast()) {
          ...
          seq.addInstruction(ConstantsUtil
              .getInstructionInstance(CodeConstants.opc_bipush, false, CodeConstants.GROUP_GENERAL,
                  bytecode_version,
                  new int[]{0}), -1);
          seq.addInstruction(ConstantsUtil
              .getInstructionInstance(CodeConstants.opc_istore, false, CodeConstants.GROUP_GENERAL,
                  bytecode_version,
                  new int[]{var}), -1);
    ...
    seq.addInstruction(
        ConstantsUtil
            .getInstructionInstance(CodeConstants.opc_bipush, false, CodeConstants.GROUP_GENERAL,
                bytecode_version, new int[]{1}),
        -1);
    seq.addInstruction(
        ConstantsUtil
            .getInstructionInstance(CodeConstants.opc_istore, false, CodeConstants.GROUP_GENERAL,
                bytecode_version, new int[]{var}),
        -1);
    ...
    while (!stack.isEmpty()) {
      ...
      if (parent != null && parent.type == Statement.TYPE_CATCHALL &&
        ...
        if (catchallBlockIDs.containsKey(handler.id)) {
          ...
          Integer var = finallyBlockIDs.get(handler.id);
          fin.setMonitor(var == null ? null
              : new VarExprent(var.intValue(), VarType.VARTYPE_INT, varProcessor));
        } else {
          ...
          if (inf == null) { // inconsistent finally
            ...
            DeadCodeHelper
                .removeDeadBlocks(graph); // e.g. multiple return blocks after a nested finally
            DeadCodeHelper.removeEmptyBlocks(graph);
            DeadCodeHelper.mergeBasicBlocks(graph);
          }
  ...
  private Record getFinallyInformation(StructMethod mt, RootStatement root,
    ...
    VarVersionPair varpaar = new VarVersionPair(
        (VarExprent) ((AssignmentExprent) lstExprents.get(firstcode == 2 ? 1 : 0)).getLeft());
        ...
        for (int i = 0; i < node.exprents.size(); i++) {
          ...
          if (firstcode == 0) {
            ...
            for (Exprent expr : lst) {
              if (expr.type == Exprent.EXPRENT_VAR && new VarVersionPair((VarExprent) expr)
                  .equals(varpaar)) {
                found = true;
                break;
              }
  ...
  private boolean verifyFinallyEx(ControlFlowGraph graph, CatchAllStatement fstat,
      Record information) {
    ...
    for (BasicBlock start : startBlocks) {
      ...
      Area arr = compareSubgraphsEx(graph, start, catchBlocks, first, finallytype, mapLast,
          skippedFirst);
      if (arr == null) {
        return false;
      }
  ...
  private Area compareSubgraphsEx(ControlFlowGraph graph,
    ...
    class BlockStackEntry {
      ...
      public BlockStackEntry(BasicBlock blockCatch, BasicBlock blockSample,
    ...
    while (!stack.isEmpty()) {
      ...
      if (!compareBasicBlocksEx(graph, blockCatch, blockSample,
          (isFirstBlock ? 1 : 0) | (isTrueLastBlock ? 2 : 0), finallytype,
          entry.lstStoreVars)) {
        return null;
      }
      ...
      if (isLastBlock && blockSample.getSeq().isEmpty()) {
        ...
        if (blockCatch.getSuccExceptions().size() == blockSample.getSuccExceptions().size()) {
          for (int i = 0; i < blockCatch.getSuccExceptions().size(); i++) {
            ...
            String excCatch = graph.getExceptionRange(sucCatch, blockCatch)
                .getUniqueExceptionsString();
            String excSample = graph.getExceptionRange(sucSample, blockSample)
                .getUniqueExceptionsString();
  ...
  public boolean equalInstructions(Instruction first, Instruction second,
      List<int[]> lstStoreVars) {
    ...
    if (first.group != CodeConstants.GROUP_JUMP
        && first.getOperands() != null) { // FIXME: switch comparison
