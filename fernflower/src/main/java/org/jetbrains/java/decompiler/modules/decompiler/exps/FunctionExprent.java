...
public class FunctionExprent extends Exprent {
  ...
  private static VarType getMaxVarType(VarType[] arr) {
    int[] types = new int[]{CodeConstants.TYPE_DOUBLE, CodeConstants.TYPE_FLOAT,
  ...
  public VarType getExprType() {
    ...
    if (funcType <= FUNCTION_NEG || funcType == FUNCTION_IPP || funcType == FUNCTION_PPI
      ...
      if (param1.type == Exprent.EXPRENT_CONST && param2.type == Exprent.EXPRENT_CONST &&
          supertype.type != CodeConstants.TYPE_BOOLEAN && VarType.VARTYPE_INT
  ...
  public CheckTypesResult checkExprTypeBounds() {
    ...
    switch (funcType) {
      case FUNCTION_IIF:
        ...
        result.addMinTypeExprent(lstOperands.get(2),
            VarType.getMinTypeInFamily(supertype.typeFamily));
        break;
      case FUNCTION_I2L:
      case FUNCTION_I2F:
      case FUNCTION_I2D:
        ...
        if (type1.type == CodeConstants.TYPE_BOOLEAN) {
          if (type2.isStrictSuperset(type1)) {
            ...
            boolean param1_false_boolean =
                type1.isFalseBoolean() || (param1.type == Exprent.EXPRENT_CONST
                    && !((ConstExprent) param1).hasBooleanValue());
            boolean param2_false_boolean =
                type1.isFalseBoolean() || (param2.type == Exprent.EXPRENT_CONST
  ...
  public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
    ...
    switch (funcType) {
      ...
      case FUNCTION_NEG:
        return wrapOperandString(lstOperands.get(0), true, indent, tracer).prepend("-");
      case FUNCTION_CAST:
        return lstOperands.get(1).toJava(indent, tracer).enclose("(", ")")
    ...
    if (funcType <= FUNCTION_I2S) {
      return wrapOperandString(lstOperands.get(0), true, indent, tracer)
          .prepend("(" + ExprProcessor.getTypeName(
