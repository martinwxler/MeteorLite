...
public class SSAUConstructorSparseEx {
  ...
  private static SFormsFastMapDirect mergeMaps(SFormsFastMapDirect mapTo,
      SFormsFastMapDirect map2) {
  ...
  private void ssaStatements(DirectGraph dgraph, HashSet<String> updated, boolean calcLiveVars) {
    ...
    for (DirectNode node : dgraph.nodes) {
      ...
      boolean allow_field_propagation =
          node.succs.isEmpty() || (node.succs.size() == 1 && node.succs.get(0).preds.size() == 1);
      ...
      boolean this_updated = !mapsEqual(varmaparr[0], outVarVersions.get(node.id))
          || (outNegVarVersions.containsKey(node.id) && !mapsEqual(varmaparr[1],
          outNegVarVersions.get(node.id)));
  ...
  private void processExprent(Exprent expr, SFormsFastMapDirect[] varmaparr, Statement stat,
      ...
      case Exprent.EXPRENT_FUNCTION:
        ...
        switch (func.getFuncType()) {
          ...
          case FunctionExprent.FUNCTION_CADD:
            ...
            SFormsFastMapDirect[] varmaparrAnd = new SFormsFastMapDirect[]{
                new SFormsFastMapDirect(varmaparr[0]), null};

            processExprent(func.getLstOperands().get(1), varmaparrAnd, stat, calcLiveVars);

            // false map
            varmaparr[1] = mergeMaps(varmaparr[varmaparr[1] == null ? 0 : 1],
                varmaparrAnd[varmaparrAnd[1] == null ? 0 : 1]);
            // true map
          ...
          case FunctionExprent.FUNCTION_COR:
            ...
            SFormsFastMapDirect[] varmaparrOr =
                new SFormsFastMapDirect[]{
                    new SFormsFastMapDirect(varmaparr[varmaparr[1] == null ? 0 : 1]), null};
    ...
    if (expr.type == Exprent.EXPRENT_FIELD) {
      ...
      setCurrentVar(varmap, index, 1);
        ...
        (expr.type == Exprent.EXPRENT_ASSIGNMENT
            && ((AssignmentExprent) expr).getLeft().type == Exprent.EXPRENT_FIELD) ||
        (expr.type == Exprent.EXPRENT_NEW
            && ((NewExprent) expr).getNewType().type == CodeConstants.TYPE_OBJECT) ||
      ...
      if (expr.type == Exprent.EXPRENT_FUNCTION) {
        ...
        if (fexpr.getFuncType() >= FunctionExprent.FUNCTION_IMM
            && fexpr.getFuncType() <= FunctionExprent.FUNCTION_PPI) {
    ...
    if (varassign != null) {
      ...
      if (cardinality == 1) { // size == 1
        if (current_vers.intValue() != 0) {
          ...
          VarVersionNode prenode = ssuversions.nodes
              .getWithKey(new VarVersionPair(varindex, lastver));
  ...
  private void createOrUpdatePhiNode(VarVersionPair phivar, FastSparseSet<Integer> vers,
      Statement stat) {
    ...
    for (Integer ver : versCopy) {
      ...
      VarVersionNode prenode = ssuversions.nodes
          .getWithKey(new VarVersionPair(phivar.var, ver.intValue()));
          ...
          for (Entry<Integer, FastSparseSet<Integer>> ent : mapExitVar.entryList()) {
            for (Integer version : ent.getValue()) {
              ...
              if (phantomvar == null) {
                ...
                VarVersionEdge edge = new VarVersionEdge(VarVersionEdge.EDGE_PHANTOM, exitnode,
                    phantomnode);
    ...
    switch (stat.type) {
      ...
      case Statement.TYPE_TRYCATCH:
        ...
        for (int i = 1; i < stat.getStats().size(); i++) {
          ...
          extraVarVersions.put(dgraph.nodes.getWithKey(
              flatthelper.getMapDestinationNodes().get(stat.getStats().get(i).id)[0]).id, map);
