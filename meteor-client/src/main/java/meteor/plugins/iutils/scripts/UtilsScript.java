...
import meteor.plugins.iutils.api.Spells;
import meteor.plugins.iutils.api.*;
import meteor.plugins.iutils.game.*;
import meteor.plugins.iutils.scene.Area;
import meteor.plugins.iutils.scene.RectangularArea;
import meteor.plugins.iutils.ui.*;
import meteor.plugins.iutils.util.Util;
import meteor.plugins.iutils.walking.BankLocations;
import meteor.plugins.iutils.walking.Walking;
...
//    @Inject
    ...
    protected void equip(String name) {
        ...
        InventoryItem item = game.inventory().withName(name).first();
    ...
    protected void useItemItem(String item1, String item2) {
        InventoryItem a = game.inventory().withName(item1).first();
        InventoryItem b = game.inventory().withName(item2).filter(i -> i.slot() != a.slot()).first();
        a.useOn(b);
    }
