...
import meteor.plugins.iutils.actor.NpcStream;
import meteor.plugins.iutils.actor.PlayerStream;
import meteor.plugins.iutils.api.EquipmentSlot;
import meteor.plugins.iutils.scene.GameObjectStream;
import meteor.plugins.iutils.scene.GroundItemStream;
import meteor.plugins.iutils.scene.ObjectCategory;
import meteor.plugins.iutils.scene.Position;
import meteor.plugins.iutils.ui.EquipmentItemStream;
import meteor.plugins.iutils.ui.InventoryItemStream;
...
import static meteor.plugins.iutils.iUtils.sleep;
...
public class Game {
    ...
    public List<Position> instancePositions(Position templatePosition) {
        ...
        for (int z = 0; z < 4; z++) {
            for (int x = 0; x < 104; x++) {
                for (int y = 0; y < 104; y++) {
                    iTile tile = new iTile(this, client.getScene().getTiles()[z][x][y]);
