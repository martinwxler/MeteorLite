...
public class Pathfinder {
    ...
    public List<Position> find() {
        ...
        while (!boundary.isEmpty()) {
            Position node = boundary.removeFirst();
    ...
    private void addNeighbors(Position position) {
        ...
        for (Position transport : transports.getOrDefault(position, new ArrayList<>())) {
        ...
        if (map.w(position.x, position.y, position.z)) {
            Position neighbor = new Position(position.x - 1, position.y, position.z);
        ...
        if (map.s(position.x, position.y, position.z)) {
            Position neighbor = new Position(position.x, position.y - 1, position.z);
            if (!predecessors.containsKey(neighbor)) {
        ...
        if (map.sw(position.x, position.y, position.z)) {
            Position neighbor = new Position(position.x - 1, position.y - 1, position.z);
            if (!predecessors.containsKey(neighbor)) {
