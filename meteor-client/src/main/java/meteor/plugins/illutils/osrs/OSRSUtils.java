package meteor.plugins.illutils.osrs;
...
import meteor.plugins.illutils.osrs.wrappers.IllWidget;
import meteor.plugins.illutils.IllUtilsConfig;
import meteor.plugins.illutils.util.InteractionManager;
...
import meteor.plugins.illutils.util.actor.NpcStream;
import meteor.plugins.illutils.util.actor.PlayerStream;
import meteor.plugins.illutils.api.EquipmentSlot;
import meteor.plugins.illutils.api.scene.GameObjectStream;
import meteor.plugins.illutils.api.scene.GroundItemStream;
import meteor.plugins.illutils.api.scene.ObjectCategory;
import meteor.plugins.illutils.api.scene.Position;
import meteor.plugins.illutils.ui.EquipmentItemStream;
import meteor.plugins.illutils.ui.InventoryItemStream;
...
import static meteor.plugins.illutils.IllUtils.sleep;
...
public class OSRSUtils {
    ...
    private final Logger log = new Logger("iUtils-Game");

    IllTile[][][] tiles = new IllTile[4][104][104];
    Position base;
    ...
    public IllPlayer localPlayer() {
        return new IllPlayer(this, client.getLocalPlayer(), client.getLocalPlayer().getPlayerComposition());
    }
    ...
    public List<Position> instancePositions(Position templatePosition) {
        ...
        for (int z = 0; z < 4; z++) {
            for (int x = 0; x < 104; x++) {
                for (int y = 0; y < 104; y++) {
                    IllTile tile = new IllTile(this, client.getScene().getTiles()[z][x][y]);
    ...
    public IllTile tile(Position position) {
        ...
        return new IllTile(this, tile);
    }

    public Stream<IllTile> tiles() {
        return Arrays.stream(client().getScene().getTiles())
                .flatMap(Arrays::stream)
                .flatMap(Arrays::stream)
                .filter(Objects::nonNull)
                .map(to -> new IllTile(this, to));
    }

    public GameObjectStream objects() {
        return getFromClientThread(() -> new GameObjectStream(IllUtils.objects.stream()
                .map(o -> new IllObject(
                        this,
                        o,
    ...
    public GroundItemStream groundItems() {
        return getFromClientThread(() -> new GroundItemStream(IllUtils.tileItems.stream()
                .map(o -> new IllGroundItem(
                        this,
                        o,
    ...
    public NpcStream npcs() {
        return getFromClientThread(() -> new NpcStream(client().getNpcs().stream()
                .map(npc -> new IllNPC(this, npc, client().getNpcDefinition(npc.getId())))
                .collect(Collectors.toList())
                .stream())
        );
    }
    ...
    public PlayerStream players() {
        return getFromClientThread(() -> new PlayerStream(client().getPlayers().stream()
                .map(p -> new IllPlayer(this, p, p.getPlayerComposition()))
                .collect(Collectors.toList())
                .stream())
        );
    }

    public IllWidget widget(int group, int file) {
    ...
    public IllWidget widget(int group, int file, int child) {
        if (client.getWidget(group, file) == null) {
            return null;
        }
        return new IllWidget(this, client.getWidget(group, file).getDynamicChildren()[child]);
    }

    public IllWidget widget(WidgetInfo widgetInfo) {
    ...
    public EquipmentItemStream equipment() {
        ...
        return getFromClientThread(() -> new EquipmentItemStream(equipped.entrySet().stream()
                .map(i -> new IllEquipmentItem(this, i.getKey(), client().getItemDefinition(i.getKey().getId()), i.getValue()))
                .collect(Collectors.toList())
                .stream())
                .filter(Objects::nonNull)
        );
     ...
     */
    public IllWidget screenContainer() {
        if (client.isResized()) {
            if (varb(4607) == 0) {
