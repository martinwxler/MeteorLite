...
import meteor.plugins.illutils.api.Useable;
...
public class IllWidget implements Interactable, Useable {

    private final OSRSUtils game;
    private final Widget widget;
    Map<Integer, IllWidget> children = new HashMap<>();

    public IllWidget(OSRSUtils game, Widget widget) {
        this.game = game;
        this.widget = widget;
    }
...
//        return game.getFromClientThread(() -> widget.getIndex());
    ...
    public List<IllWidget> items() {
        ArrayList<IllWidget> items = new ArrayList<>();

        for (Widget slot : widget.getDynamicChildren()) {
            if (slot != null) {
                items.add(new IllWidget(game(), slot));
            }
        }
        return items;
    }
...
//        );
    ...
    public Widget child(int child) { //TODO untested
        Widget c = widget.getDynamicChildren()[child];
    ...
    @Override
    public void useOn(IllInventoryItem item) {
    ...
    @Override
    public void useOn(IllObject object) {
        game.interactionManager().submit(() -> game.clientThread.invoke(() -> {
            game.client.setSelectedSpellWidget(id());
            game.client.setSelectedSpellChildIndex(-1);
