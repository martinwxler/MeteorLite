...
public enum RuneElement {
    ...
    public int getCount(OSRSUtils game) {
        if (haveStaff(game)) {
            return Integer.MAX_VALUE;
        }
        List<IllInventoryItem> runes = game.inventory().withNamePart("rune").all();
        List<IllInventoryItem> items = new ArrayList<>();

        for (IllInventoryItem rune : runes) {
        ...
        return items.stream().mapToInt(IllInventoryItem::quantity).sum() + RunePouch.getQuantity(game, this);
    }

    private boolean haveStaff(OSRSUtils game) {
        IllEquipmentItem item = game.equipment().withNamePart("staff").first();
