...
import meteor.plugins.illutils.ui.StandardSpellbook;
...
public class TeleportLoader {
    ...
    private final Logger log = new Logger("iUtils-Teleporter");

    public TeleportLoader(OSRSUtils game) {
        this.game = game;
...
//        }
            ...
            if (OSRSUtils.getWildernessLevelFrom(game.client().getLocalPlayer().getWorldLocation()) <= 20) {
    ...
    //Jewellery
    private void jewelleryAction(IllInventoryItem item, String... target) { // TODO
        item.interact("Rub");
        game.tick();
        chatbox.chat(target);
    }

    private void jewelleryContainerAction(IllInventoryItem item, String target) {
        item.interact("Rub");
        game.tick();
