...
public class TransportLoader {
    ...
    public static final List<SpiritTree> SPIRIT_TREES = Arrays.asList(new SpiritTree(new Position(2542, 3170, 0), "Tree gnome Village"),
    ...
    public static List<Transport> buildTransports(OSRSUtils game) {
        ArrayList<Transport> transports = new ArrayList<>();

        try {
            for (String line : new String(readAllBytes(TransportLoader.class.getResourceAsStream("/transports.txt"))).split("\n")) {
                line = line.trim();
        ...
        if (game.varp(111) == 9 && game.membersWorld()) {
            for (SpiritTree source : SPIRIT_TREES) {
                ...
                for (SpiritTree target : SPIRIT_TREES) {
...
//        3625 3224 0 3625 3223 0 Open Door 18057
        ...
        List<Position> icyeneGraveyardSources = Arrays.asList(new Position[]{new Position(3684, 3176, 0), new Position(3684, 3175, 0), new Position(3684, 3174, 0)});
        Position icyeneGraveyardTarget = game.varb(7255) <= 44 ? new Position(3684, 3181, 0) : new Position(3684, 3176, 0);

        List<Position> meiyerditchSources = Arrays.asList(new Position[]{new Position(3605, 3161, 1), new Position(3605, 3163, 1)});
        Position meyerditchTarget = new Position(3605, 3161, 1);

        Position burghDeRottSource = new Position(3525, 3170, 0);
        Position burghDeRottTarget = new Position(3525, 3170, 0);

        if (game.varb(7255) < 42) {
            transports.add(objectTransport(burghDeRottSource, meyerditchTarget, 12945, "Board"));

            for (Position meiyerditchSource : meiyerditchSources) {
            ...
            for (Position source : icyeneGraveyardSources) {
...
//        3672 3374 0 3670 3375 0 Climb Wall 39541
    ...
    private static Transport objectChatTransport(Position source, Position target, int id, String action, String... options) {
        return new Transport(source, target, Integer.MAX_VALUE, 0, game -> {
            game.objects().withId(id).nearest(source).interact(action);
            Chatbox chatbox = new Chatbox(game);
