...
//    @Inject
    ...
    private Logger log = new Logger("UtilsScript");
    ...
    protected void equip(String name) {
        ...
        IllInventoryItem item = game.inventory().withName(name).first();
    ...
    protected void obtain(List<ItemQuantity> items) {
        ...
        obtain(items.toArray(new ItemQuantity[0]));
    }
    ...
    protected void obtainBank(List<ItemQuantity> items) {
        obtainBank(items.toArray(new ItemQuantity[0]));
    }
    ...
    protected void obtainBank(ItemQuantity... items) {
        ...
        List<IllWidget> bankItems = bank().items();
    ...
    private int bankItemQuantity(List<IllWidget> bankItems, int id) {
        IllWidget bankItem = bankItems.stream()
    ...
    protected Depositbox depositbox() {
        Depositbox depositbox = new Depositbox(game);
    ...
    protected GrandExchange grandExchange() {
        ...
        GrandExchange grandExchange = new GrandExchange(game);
    ...
    public void killNpc(IllNPC npc, Prayer... prayers) {
    ...
    protected void castSpellNpc(IllNPC npc, Spells spell) {
        if (npc != null) {
    ...
    protected void castSpellItem(IllInventoryItem it, Spells spell) {
        if (it != null) {
    ...
    protected void useItemItem(String item1, String item2) {
        IllInventoryItem a = game.inventory().withName(item1).first();
        IllInventoryItem b = game.inventory().withName(item2).filter(i -> i.slot() != a.slot()).first();
        a.useOn(b);
    }
