...
import meteor.plugins.illutils.api.ItemQuantity;
...
public class Bank {
    ...
    private final Logger log = new Logger("iUtils-Bank");

    @Inject
    public Bank(OSRSUtils game) {
        this.game = game;
    }
    ...
    public int withdraw(int id, int quantity, boolean noted) {
        ...
        int inventoryCapacity = inventoryCapacity(noted ? definition.getLinkedNoteId() : id);

        if (inventoryCapacity == 0) {
            return 0;
        }

        for (IllWidget item : IllUtils.bankitems) {
            if (item.itemId() == id) {
    ...
    public void depositExcept(boolean delay, Collection<Integer> ids) {
        List<IllInventoryItem> items = game.inventory().withoutId(ids).all();
        Set<Integer> deposited = new HashSet<>();
        while (game.inventory().withoutId(ids).exists()) {
            for (IllInventoryItem item : items) {
    ...
    public boolean deposit(int id, int quantity) {
        ...
        for (IllWidget item : IllUtils.bankInventoryitems) {
            if (item.itemId() == id) {
...
//    }
    ...
    public List<IllWidget> items() {
        if (IllUtils.bankitems.isEmpty()) {
            game.tick(); //Give time for items to load
        }
        return IllUtils.bankitems;
    }
...
//            }
    ...
    private int inventoryCapacity(int id) {
        ...
        if (stackable) {
            IllInventoryItem item = game.inventory().withId(id).first();
