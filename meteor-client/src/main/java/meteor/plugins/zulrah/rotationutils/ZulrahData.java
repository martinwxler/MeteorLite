...
public class ZulrahData {
  ...
  public Optional<StandLocation> getCurrentDynamicStandLocation() {
    if (current == null) {
      ...
      switch (current.getAttributes().getStandLocation()) {
        case NORTHEAST_TOP:
          return ZulrahPlugin.isFlipStandLocation() ? Optional.of(StandLocation.NORTHEAST_BOTTOM)
              : Optional.of(current.getAttributes().getStandLocation());
        case WEST:
          return ZulrahPlugin.isFlipStandLocation() ? Optional.of(StandLocation.NORTHWEST_BOTTOM)
              : Optional.of(current.getAttributes().getStandLocation());
        default:
  ...
  public Optional<Prayer> getCurrentPhasePrayer() {
    if (ZulrahPlugin.isZulrahReset()) {
      ...
      return isJad() ? (ZulrahPlugin.isFlipPhasePrayer() ? Optional.of(invertedPhasePrayer)
