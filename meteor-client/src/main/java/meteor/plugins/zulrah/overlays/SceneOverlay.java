...
import meteor.plugins.zulrah.rotationutils.ZulrahNpc;
...
public class SceneOverlay extends Overlay {
  ...
  @Inject
  private SceneOverlay(Client client, ZulrahPlugin plugin, ZulrahConfig config,
  ...
  private void renderZulrahPhaseTiles(Graphics2D graphics) {
    if (config.phaseDisplayType() != ZulrahConfig.DisplayType.OFF
      ...
      SetMultimap<ZulrahLocation, MutablePair<String, ZulrahNpc>> zulrahLocationsGrouped = LinkedHashMultimap
          .create();
    ...
    if (zulrahNpc != null) {
      ...
      if (basePoint != null) {
        ...
        if (this.config.phaseHats()) {
          BufferedImage icon = skillIconManager
              .getSkillImage(zulrahNpc.getType().getSkill(), this.config
        ...
        if (config.standLocations()) {
          if (data.standLocationsMatch()) {
            data.getCurrentDynamicStandLocation().ifPresent((loc) -> {
              standLocationsGrouped
                  .put(loc, new MutablePair("Stand / Next", config.standAndNextTileColor()));
            });
          } else {
            data.getCurrentDynamicStandLocation().ifPresent((loc) -> {
  ...
  private void renderPrayerConservation(Graphics2D graphics) {
    if (config.prayerConservation()) {
      ...
      if (player != null && (plugin.getZulrahNpc().getInteracting() == null
        ...
        Point textLocation = player
            .getCanvasTextLocation(graphics, "Turn off overheads to conserve prayer!",
                player.getLogicalHeight() - 22);
        Color color = Color.RED;
        ZulrahConfig config = this.config;
        Objects.requireNonNull(config);
        OverlayUtils
            .renderTextLocation(graphics, textLocation, "Turn off overheads to conserve prayer!",
                color, config::textOutline);
      }
  ...
  private void renderZulrahTicks(Graphics2D graphics) {
    if (config.phaseTickCounter() || config.attackTickCounter()) {
      ...
      sb = sb.append(
          config.phaseTickCounter() && plugin.getPhaseTicks() >= 0 ? plugin.getPhaseTicks() : "")
          .append(
  ...
  private void renderZulrahTile(Graphics2D graphics) {
    if (config.displayZulrahTile()) {
      Polygon tileAreaPoly = Perspective
          .getCanvasTileAreaPoly(client, plugin.getZulrahNpc().getLocalLocation(), 5);
      OverlayUtils
          .renderPolygon(graphics, tileAreaPoly, config.zulrahTileColor(), config.outlineWidth(),
              config.fillAlpha());
    }
  }
  ...
  private void renderProjectiles(Graphics2D graphics) {
    if (config.displayProjectiles() && plugin.getProjectilesMap().size() > 0) {
      plugin.getProjectilesMap().forEach((localPoint, ticks) -> {
        Point textLocation = Perspective
            .getCanvasTextLocation(client, graphics, localPoint, "#", 0);
  ...
  private void renderToxicClouds(Graphics2D graphics) {
    ...
    plugin.getToxicCloudsMap().forEach((obj, ticks) -> {
      ...
      OverlayUtils
          .renderPolygon(graphics, tileAreaPoly, config.toxicCloudsColor(), config.outlineWidth(),
              config.fillAlpha());
      String ticksString = Integer.toString(ticks);
      Point textLocation = Perspective
          .getCanvasTextLocation(client, graphics, localPoint, ticksString, 0);
